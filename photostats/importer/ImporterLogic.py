...
def do_import(path, subdir=False, files_scan=False):
    ...
    logger.info("Import started: " +
                str(datetime.now()) + " on folder " + path)

    # the above is separated from the actual import below to keep
    # the init-stuff out of the recursion
...
def import_folder(path, parent, logger, subdir=False, files_scan=False):
    ...
    if subdir:
        ...
        for dir in os.listdir(path):
            ...
            sub_imgs = import_folder(
                subdirpath, myDir, logger, subdir, files_scan)
