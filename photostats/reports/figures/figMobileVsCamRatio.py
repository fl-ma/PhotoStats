...
class MobileVsCamRatio(FigParent):
    ...
    def calculate(self):
        ...
        photos_cam = photos.values(
            'date_taken__date', 'camera__camera_model').annotate(
            total=Count('date_taken__date')).order_by('date_taken__date')

        photos_tot = photos.values('date_taken__date').annotate(
            total=Count('date_taken__date')).order_by('date_taken__date')
        ...
        for photo_cam in photos_cam:
            ...
            if count_total == 0 and photo_cam.get('total') > 0:
                raise ValueError('No total found for: ' + photo_cam.get(
                    'date_taken__date') + 'but photos from ' + cam_model)
