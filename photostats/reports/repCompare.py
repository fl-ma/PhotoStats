...
class RepCompare(RepParent):
    ...
    def build(self):
        ...
        if not action:
            # initialize view
            self.context = {
                'FilterLeft':  selLeft.get_html(
                    div_id='FilterLeft',
                    selector_name=SEL_LEFT, title='Directory 1'),
                'FilterRight': selRight.get_html(
                    div_id='FilterRight',
                    selector_name=SEL_RIGHT, title='Directory 2'),
            }
    ...
    def plot_reports(self):
        ...
        focalDonutLeft = FocalLengthDonut(selLeft.get_selected_as_filter(
        ), 'focalDonutLeft', title='Focal length distribution')

        # right column
        selRight = DirectorySelector(self.request.POST.get(SEL_RIGHT))

        focalDonutRight = FocalLengthDonut(selRight.get_selected_as_filter(
        ), 'focalDonutRight', title='Focal length distribution')

        self.context = {
            'FilterLeft': selLeft.get_html(
                div_id='FilterLeft',
                selector_name=SEL_LEFT,
                title='Directory 1'),
            'FilterRight': selRight.get_html(div_id='FilterRight',
                                             selector_name=SEL_RIGHT,
                                             title='Directory 2'),
